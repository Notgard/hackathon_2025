...
void arm_random_number_generator(real * random_numbers, size_t size, VSLStreamStatePtr stream)
{
...
real black_scholes_monte_carlo(ui64 S0, ui64 K, real T, real r, real sigma, real q, ui64 num_simulations, VSLStreamStatePtr stream)
{
    real sum_payoffs = 0.0;
    //real sigma_square = sigma * sigma;
    
    real T_sqrt = sigma * real_sqrt(T);
    real temp_val = (r - q - 0.5 * sigma * sigma) * T;

    real *random_numbers = (real *)malloc(num_simulations * sizeof(real));

    arm_random_number_generator(random_numbers, num_simulations, stream);

    for (ui64 i = 0; i < num_simulations; ++i)
    {
        real Z = random_numbers[i];
        // double ST = S0 * exp((r - q - 0.5 * sigma * sigma) * T + sigma * sqrt(T) * Z);
        real ST = S0 * exp(temp_val + T_sqrt * Z);
        real payoff = std::max<real>(ST - K, 0.0);
        sum_payoffs += payoff;
    }
    free(random_numbers);
...
__m256 exp256_ps(__m256 x) {
    ...
    _mm256_storeu_ps(x_array, x);
    for (int i = 0; i < VECTOR_SIZE; ++i) {
        result_array[i] = std::exp(x_array[i]);
    }
    return _mm256_loadu_ps(result_array);
}
...
{
    __m256 S0_vec = _mm256_set1_ps(S0);             // Broadcast S0 to all lanes
    __m256 temp_val_vec = _mm256_set1_ps(temp_val);   // Broadcast temp_val
    __m256 T_sqrt_vec = _mm256_set1_ps(T_sqrt);     // Broadcast T_sqrt
    __m256 K_vec = _mm256_set1_ps(K);               // Broadcast K
    __m256 zero_vec = _mm256_setzero_ps();          // Zero vector
    ...
    for (size_t i = 0; i < num_simulations; i += VECTOR_SIZE) {
        ...
        __m256 Z_vec = _mm256_loadu_ps(&random_numbers[i + offset]); // Unaligned load

        // Calculate exponent = temp_val + T_sqrt * Z
        __m256 exponent = _mm256_fmadd_ps(T_sqrt_vec, Z_vec, temp_val_vec);

        // Calculate ST = S0 * exp(exponent)
        __m256 exp_result = exp256_ps(exponent);     // Vectorized exp function
        __m256 ST_vec = _mm256_mul_ps(S0_vec, exp_result);

        // Calculate payoff = max(ST - K, 0.0)
        __m256 payoff_vec = _mm256_max_ps(_mm256_sub_ps(ST_vec, K_vec), zero_vec);

        // Accumulate payoffs into a local sum
        float sum_array[VECTOR_SIZE];
        _mm256_storeu_ps(sum_array, payoff_vec);
...
int main(int argc, char *argv[])
{
    ...
    int errcode = vslNewStream(&stream, VSL_BRNG_MT19937, 1);
